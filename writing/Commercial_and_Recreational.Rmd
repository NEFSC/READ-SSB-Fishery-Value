...
stock_keyfile<-CAMS_revenue %>%
  select(c(council, fmp, itis_name, itis_tsn, stockarea)) %>%
  group_by(council, fmp, itis_name, itis_tsn, stockarea) %>% 
    slice(1) %>%
  ungroup() %>%
  arrange(council,fmp, itis_name, stockarea)
...
REC_DATA<-REC_DATA %>%
  mutate(stockarea = case_when(
         itis_tsn %in% out_of_area_species ~ "Unit",
         .default=stockarea)
)
...
#   filter(missing==FALSE & target_common=="BLACK SEA BASS")
 ...
 test2<-REC_DATA %>% 
   filter(missing==TRUE & prim1_common=="ATLANTIC COD")
...
```{r format_and_export}
Commercial<-CAMS_revenue %>%
  select(-commercial_landed_pounds) %>%
  pivot_wider(names_from=year, values_from=commercial_value) %>%
  relocate(council, fmp, itis_tsn, itis_sci_name, itis_name, area_name2)

saveRDS(Commercial, file=here("data_folder","main",glue("Commercial_Expenditures_{params$finished_switch}_{vintage_string}.Rds")))
