...
```{r ASFMC_stocks}
 knitr::kable(stock_keyfile %>%
  filter(council == "ASMFC" | council=="ASMFC/NEFMC")
  %>% select(-c(council, itis_tsn)),
  caption='ASMFC stocks',format.args = list(big.mark = ","), digits=1, align=c("l", "l","l","l") ) 
...
```{r MAFMC_stocks}
 knitr::kable(stock_keyfile %>%
  ...
  select(-c(council, itis_tsn)),
  caption='MAFMC stocks',format.args = list(big.mark = ","), digits=1, align=c("l", "l","l","l") ) 
...
```{r NEFMC_stocks}
 ...
 knitr::kable(stock_keyfile %>%
  ...
  select(-c(council, itis_tsn)), 
  caption='NEFMC stocks',format.args = list(big.mark = ","), digits=1, align=c("l", "l","l","l") ) 
...
```{r HMS_SERO}
 knitr::kable(fmp_listing %>%
  filter(fmp  %in% c("SERO FMP","Highly Migratory Species"))  %>% select(c(fmp, itis_name)), caption='HMS and SERO stocks',format.args = list(big.mark = ","), digits=1, align=c("l", "l","l","l") ) 
...
```{r Non-Managed}
 knitr::kable(stock_keyfile %>%
  filter(fmp=="No Federal FMP")  %>%
  arrange(itis_name) %>%
  select(-c(council, itis_tsn)), caption='Non-Northeast Region stocks',format.args = list(big.mark = ","), digits=1, align=c("l", "l","l","l") ) 
...
impact.data0.2022.mod <- impact.data0.2022 %>%
  left_join(impact.data0.2022.1, by = join_by(mode==mode), relationship="many-to-one")

stopifnot(nrow(impact.data0.2022)==nrow(impact.data0.2022.mod))

impact.data0.2022.mod <- impact.data0.2022.mod %>%
  left_join(states,by = join_by(state==state), relationship="many-to-one")%>%
...
knitr::kable(filtered_states,digits=2,
             col.names = c("State","Mode","Expenditure/Trip","Adult Share","Value-Added Multiplier"),
             align=c("l", "l","r","r","r"),
            caption='Expenditure per adult trip, adult share, and value added multipliers') 
...
# for each county, pick the most common statistical area, in case there are intsite in the MRIP
# data that do not appear in the site_list.
...
tsns<-fmp_listing %>%
  select(itis_tsn,fmp,council,itis_name, itis_sci_name) %>%
  mutate(in_fmp_keyfile=1)

# Inner join to the keyfile to keep just the trips that are in the fmp listing 

Tripdata  <- Tripdata %>% 
  inner_join(tsns, by=join_by(tsn1==itis_tsn), relationship="many-to-one")
...
start_length<-nrow(Tripdata)

Tripdata  <- Tripdata %>% 
  left_join(site_list, by=join_by(st==state_code, cnty==county_code,intsite==intsite), relationship="many-to-one")

# use the most prevalent stat area by county in case there are missing intsites
Tripdata  <- Tripdata %>% 
  left_join(county_list, by=join_by(st==state_code, cnty==county_code), relationship="many-to-one")
...
REC_DATA<-Tripdata %>%
  left_join(stock_area_definitions, by=join_by(tsn1==itis_tsn, nmfs_stat_area==area), relationship="many-to-one")

stopifnot(nrow(REC_DATA)==start_length)

# This is code used to look at any species market as No Federal FMP 
# test2<-REC_DATA %>%
#   filter(fmp %in% "No Federal FMP")
# 
# table(test2$prim1_common)
# This is code used to look at any species I have left with out a stockarea.  
# test3<-REC_DATA %>%
...
# This may or may not be correct, but for these purposes it doesn't matter.
REC_DATA<-REC_DATA %>%
...
# If our data is good, nothing is missing
...
```{r compute_trips, eval=TRUE}
# This takes a while, so set eval=TRUE only for the first time you run.
# Compute total trips targeting a stock by mode. I'm keeping mode because expenditures vary by mode. 
RecreationalTrips <- svyby(~dtrip, by=~year+st+tsn1+mode_fc+stockarea,
                     svytotal,
                     design=Effortdesign)

# Round, convert mode_fc to character
RecreationalTrips<-RecreationalTrips %>%
  mutate(dtrip=ceiling(dtrip),
         se = ceiling(se),
         mode_str= as.character(mode_fc)
)

saveRDS(RecreationalTrips, file=here("data_folder","main",glue("RecreationalTrips_{vintage_string}.Rds")))
...
where $D_{i,t,s}$ is the total target trips, $a_{i,t}$ is the share of adults, $e_{i,t}$ is the expenditure per adult trip, and $m_{i}$ is the value-added multiplier. To implement this formula we combine the trips and the expenditure impact data and perform the calculations. 


```{r compute_rec_expenses}
RecreationalTrips<-readRDS(file=here("data_folder","main",glue("RecreationalTrips_{vintage_string}.Rds")))

RecreationalTrips <- RecreationalTrips %>%
  left_join(impact.data0.2022.mod,by = join_by(st==statefp, mode_str==mode), relationship="many-to-one") %>%
  mutate(
...
RecreationalTrips<-RecreationalTrips %>%
  ...
  left_join(fmp_listing, by=join_by(itis_tsn==itis_tsn), relationship="many-to-one") %>%
  relocate(council, fmp, itis_tsn, itis_sci_name, itis_name, stockarea, st, stateabb, statefp_string)%>%
  arrange(council, fmp, itis_name,stockarea, year, st) %>%
  rename(manager=council) %>%
  select(-nespp3)


saveRDS(RecreationalTrips, file=here("data_folder","main",glue("Disaggregated_Recreational_Trips_{rec_vintage_string}.Rds")))

write.csv(x = RecreationalTrips, file=here("data_folder","main",glue("Disaggregated_Recreational_Trips_{rec_vintage_string}.csv")))
...
```{r save}	
  # Aggregate to the year and stock level.
Recreational_Expenditures<-RecreationalTrips %>%
  group_by(year,itis_tsn,stockarea) %>%
...
# Keep only the rows in the Northeast

Recreational_Expenditures_NER<-Recreational_Expenditures %>%
  filter(is.na(manager)==0)

saveRDS(Recreational_Expenditures_NER, file=here("data_folder","main",glue("Recreational_Expenditures_NERmanaged_{rec_vintage_string}.Rds")))

write.csv(x = Recreational_Expenditures_NER, file=here("data_folder","main",glue("Recreational_Expenditures_NERmanaged_{rec_vintage_string}.csv")))


# Average over the 2018-2024 time period. Excuse the hard coded 7.

Recreational_Expenditures_NER<-Recreational_Expenditures_NER %>%
  group_by(manager, fmp, itis_tsn, itis_sci_name, itis_name, stockarea) %>%
  summarise(avg_ValueAdded_maxyr=sum(ValueAdded_maxyr)/7,
            trips=sum(trips)/7)


saveRDS(Recreational_Expenditures_NER, file=here("data_folder","main",glue("Recreational_Expenditures_avg_NER_managed{rec_vintage_string}.Rds")))

write.csv(x = Recreational_Expenditures_NER, file=here("data_folder","main",glue("Recreational_Expenditures_avg_NER_managed{rec_vintage_string}.csv")))

# Print the tables
Recreational_Expenditures_NER <- Recreational_Expenditures_NER %>%
  mutate(avg_ValueAdded_maxyr=dollar(avg_ValueAdded_maxyr, accuracy=1 )) %>%
  ungroup()


 knitr::kable(Recreational_Expenditures_NER  %>%
    select(-c(itis_tsn, itis_sci_name)),
    col.names = c("Manager","FMP","Species Name","Stock","Value-Added","Trips"),
    caption='Average Value-Added and Trips 2018-2024',format.args = list(big.mark = ","), digits=0, align=c("l", "l","l","l","r","r") ) 
